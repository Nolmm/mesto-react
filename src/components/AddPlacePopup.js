...
function AddPlacePopup(props) {
  ...
  return (
    <PopupWithForm name="add-card" title="Новое место" Formname="add-form" children={
      <>
        <input onChange={handleNameinput} type="text" name="placename" id="place-name-input" placeholder="Название" className="popup__item popupadd__item_place-name" required minLength="1" maxLength="30" />
        <span id="place-name-input-error" className="popup__input-error">Вы пропустили это поле</span>
        <input onChange={handleLinkInput} type="url" name="placeimg" id="place-img-input" placeholder="Ссылка на картинку" className="popup__item popupadd__item_place-img" required />
